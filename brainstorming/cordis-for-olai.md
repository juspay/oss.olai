...
2. **The app on Cordis, whole** ([juspay/olai#474](https://github.com/juspay/olai/pull/474), open). One lane, in this order, each step green alone, and the lane merges as one unit or not at all. *Ruled by the human, 2026-09-02: olai is one app, not a client and a server; a PR that finishes the server half and covers the seam with a test does not merge, and a hand-written list held equal to a data file by a test is duplication, not a fix.* The server steps below landed on the branch first; the browser steps were pulled forward from what was phase 5 and belong to the same unit.
   ...
   - **Not started in this lane**: HMR (no Bun cache bust exists, §5); intercept in the store (it belongs on the `vault` service, §8); node-agent scopes (phase 5); the kind vocabulary's boot-time snapshot in `propKinds.ts` (phase 6's debt). Retire `.worktrees/cordis-spike` when the draft closes.
3. **The plugin API is Effect, and a plugin is one directory.** One lane, touching every plugin once, before the agents lane writes three more against whatever API exists then. Two halves, and the first is the one the human asked for in these words: *olai uses Effect; for an architecture with no hacks or escape hatches, plugins are written in Effect and Cordis is an engine nobody outside `@olai/plugin-api` sees.*
   ...
   - **Services are Effect services.** `ctx.vault`, `ctx.kinds`, `ctx.surfaces`, `ctx.deliveries`, `ctx.wakes`, `ctx.watching`, `ctx.held`, `ctx.env`, `ctx.clock`, `ctx.log` become `Context.Tag`s whose implementations are provided by the facade from the Cordis-side services; `ctx.log` is Effect's logger; `deliver` and `register` return Effects; a registration's undo is a finalizer on the calling plugin's scope. Events become `Stream`s (`vault/revision`, `vault/unloaded`, `surfaces/published`) and the waterfall (`chat/session-start`) becomes Effect middleware. The stamp still comes from the fiber: the facade reads `ctx.fiber.name` once and provides it as the plugin's identity.
   - **Cordis is confined.** No package outside `@olai/plugin-api` imports `cordis` or `@cordisjs/*`. The `cordis` arm in `fence.test.ts`'s `FRAMEWORK` shrinks from "importable anywhere" to "one package", and `declare module 'cordis'` lives there alone. The loader still mounts a module by name from a row; what the row points at is the facade's wrapper of the plugin's Effect, exported from the plugin's `server.ts`. Cordis stays the engine for the reactive half (activation and re-resolution) and the loader (rows, include, patches, per-field reconciliation), which is the part worth not writing again; the alternative, an Effect-native registry and loader, stays named as the other consistent answer and is not taken.
   - **One directory per plugin.** `packages/kolu-client` and `packages/odu-client` fold into `olai-plugin-kolu` and `olai-plugin-odu` (as `src/appliance/`, or a `./appliance` subpath if a second door is needed), and the two packages are deleted. They exist because, before phase 2, a plugin could not import the interface and the only way to get an appliance out of core was a package that depended on nothing; that reason is gone. They are also the one place the fence's claim is bent by naming rather than by import. xyne-spaces, which arrived after the runtime, is one directory and is the shape. `fence.test.ts` and `check-hydrated-deps.sh` follow the move.
   - **Done means**: three plugins written as Effects with no `cordis` import; the composition root's `Effect.promise` wrappers and the services' `ring(...)` callbacks gone; the bridge is one facade, in one file, with its own tests; the two client packages deleted; the fence's `cordis` arm narrowed and prove-fence catching a plugin that imports `cordis` directly.
4. **Agents as plugins** (the roster lane, unchanged in intent): `olai-plugin-claude` / `-opencode` / `-pi`, each registering on `ctx.agents`; `AGENTS` in `@olai/surface` becomes the data that registry emits.
5. **Node agents as scopes.** `createScope` per node; sessions are fibers; the subtree write fence is `intercept` metadata on `vault`; reaping is dispose; derived wakes are the session subscribing to `vault/revision` filtered by its subtree. Node-agents phase 1 (roster and switching) is already on master; this phase delivers node-agents phase 2 (concurrency) and phase 4 (derived wakes) as lifecycle rather than as a map in `agent.ts`.
6. **Loader surface**: `olai --dump-config`, `olai plugin add`, a preferences page that writes `disabled` onto rows (the nix option writes the same overlay, so both stay honest about which one spoke), and the kind vocabulary re-read when a fiber comes or goes, which is what a live toggle needs. HMR in dev only if a Bun cache bust is found (§5); until then a config edit that flips `disabled` is the reload.
...
| Engine legs (`chat/src/agents/{claude,opencode,pi}`) | ~1,500 | 2 | one plugin per engine on `ctx.agents` | Already phase 4 |
