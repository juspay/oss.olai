...
1. **Spike** (a week, throwaway). Add the `cordis` pin to `npins/sources.json` and a `nix/cordis.nix` seeding `cordis` core and `@cordisjs/plugin-loader`, hydrated through the existing script; `check-hydrated-deps.sh` runs against it from the first commit. Mount `olai-plugin-odu` as a fiber with `inject = ['vault','deliveries','kinds','surfaces']`, `ctx.surfaces` re-composing on register/dispose and republishing the roster cell. Prove: `disabled: true` at runtime leaves core's tags byte-identical (`composition.test.ts`'s claim, at every moment); a throwing `apply` lands `FAILED` with kolu unaffected; `--plugins` is a patch. Record whether `@kolu/surface` needs anything for live sibling changes and whether HMR works under Bun.
