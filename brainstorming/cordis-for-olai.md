...
4. **The plugin API is Effect** (next). One PR, a content diff on every plugin's `server.ts`, reviewed on the final paths phase 3 gave them, and before the agents lane writes three more plugins against whatever API exists then. *The human, 2026-09-02: olai uses Effect; for an architecture with no hacks or escape hatches, plugins are written in Effect and Cordis is an engine nobody outside one package sees.*
