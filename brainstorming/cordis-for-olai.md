...
**HMR on Bun.** dsh runs HMR on Node under `tsx`, clearing both ESM and CJS caches. Bun's module cache is a spike question.

## 6. The plan

Each phase is one PR, green alone.

1. **Spike** (a week, throwaway). Vendor Cordis core + loader. Mount `olai-plugin-odu` as a fiber with `inject = ['vault','deliveries','kinds','surfaces']`, `ctx.surfaces` re-composing on register/dispose and republishing the roster cell. Prove: `disabled: true` at runtime leaves core's tags byte-identical (`composition.test.ts`'s claim, at every moment); a throwing `apply` lands `FAILED` with kolu unaffected; `--plugins` is a patch. Record whether `@kolu/surface` needs anything for live sibling changes and whether HMR works under Bun.
2. **Server on Cordis.** `PluginServices` dissolves into services (`vault`, `env`, `clock`, `log`, `deliveries`, `kinds`, `surfaces`, `wakes`); `revision`/`unloaded`/`published` become `vault/revision`, `vault/unloaded`, `surfaces/published`; `probe` becomes the `chat/session-start` waterfall; the three rosters become the base bundle; `plugin-api` shrinks to the service definitions.
...
6. **Loader surface**: HMR in dev, `olai --dump-config`, `olai plugin add`, a preferences page that writes `disabled` onto rows (the nix option writes the same overlay, so both stay honest about which one spoke).
...
| The receptacle stays home; `packages/plugins` never goes upstream | **Reframe** | Nothing of olai's goes upstream. Cordis comes *down*, vendored, the way dsh did it. |
...
## 8. Questions the spike answers

- Can `@kolu/surface` add and drop a sibling on a live group, and does `connectSurfaces` tolerate a roster change, or does the client need a reconnect per change?
- Does Cordis's `Proxy`-mediated `ctx` cooperate with Effect fibers, and does HMR's cache clearing work under Bun?
- Which registrations are revertible in fact (surface, kinds, dressings, wakes, listeners: all) and which are emissions (vault writes, deliveries, socket dials) that need a compensation story or none?
- Where the intercept metadata for the subtree fence is consulted: the store's write path, or a `vault` service wrapping it.
- How the browser Cordis app boots: from the roster cell, or from a served config filtered to the wire and browser doors.
