...
5. **Agents as plugins** ([juspay/olai#486](https://github.com/juspay/olai/pull/486), in review; one PR, dispatched 2026-09-03 to claude-opus on `agents-as-plugins`, manual merge). One plugin per engine, never a roster plugin: the three rows share no release clock (Claude's adapter pin moved five times in a month, opencode's zero), `--plugins` enables them one at a time, and the fence gets one more general package that spells no engine.
