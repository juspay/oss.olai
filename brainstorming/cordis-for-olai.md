...
3. **A plugin is one directory, then the plugin API is Effect.** Two PRs in this order, both before the agents lane writes three more plugins against whatever API exists then. **3a** is the move: a rename diff git shows as renames, reviewed as paths and imports and nothing else. **3b** is the API: a content diff on every plugin's `server.ts`, reviewed on its final paths. One PR for both would defeat rename detection and hand the reviewer additions and deletions with no way to tell the move from the rewrite; each is whole on its own by the ruling (after 3a every plugin is one directory; after 3b every plugin is Effect and Cordis is confined). Two halves, and the first is the one the human asked for in these words: *olai uses Effect; for an architecture with no hacks or escape hatches, plugins are written in Effect and Cordis is an engine nobody outside `@olai/plugin-api` sees.*
