...
| The palette's Agents rows | `olai-plugin-navigation` owns the `paletteAdapters` registry; chat owns its one adapter, scoped to its browser activation | chat registers with scoped cleanup; navigation draws the rows and calls back on selection |
| The Inbox file's path | `olai-plugin-capture`, which owns an entry in an always-available registry with scoped cleanup (§5.9a) | chat reads the registry and reads absence from it; nothing in chat waits on capture |
...
| The writer vocabulary (`web`, `mcp`, `chat-agent`, `auto`, and the new `filer`) | `@olai/format` (`src/committing.ts`) | a static contract; `Ops.run` takes the writer, the Commit panel and the ledger draw its label |
...
**4.2 Where the server's "foreground" is used.** `grep -rn "selected\|foreground\|loadSession" packages/plugins/chat/src/server.ts packages/plugins/chat/src/chat.ts packages/plugins/chat/src/scoped.ts`. The notion of one selected conversation per tab goes away; "selected conversations prevent timed eviction" (`docs/chat.md:600`) becomes "conversations any tab is reading prevent timed eviction". Keep pending questions and active background tasks as eviction blockers exactly as they are. **The boot-time auto-open goes with it**: today the server opens the conversation the which-conversation note names, else the newest stored one, as its foreground at boot (`docs/chat.md:72-78`). That existed so the dock had something to show, and after this PR nothing reads a foreground, so a boot opens no conversation; every stored chat, filed or not, is asleep until a tab unfolds it, a question or background task holds it, or a wake acts on it. The which-conversation note under `~/.local/state/olai/` carries two things today: which conversation to come back to, and the model that conversation was put on (`docs/chat.md:78, 113-128`). Only the first is retired. The note stops being written and its conversation half is ignored when present (do not delete files a person may still have; say in `docs/chat.md` that it is no longer read). The **model choice moves to a store of its own**, keyed by `Conversing` (`{engine, session}`), kept beside `heard/` in the same local-state mechanism (`local.ts`), with the same thirty-two-conversation cap and least-recently-touched eviction, written when a model switch is confirmed for a conversation and **read only when that conversation is opened** (a tab subscribing to it, 5.2), never at boot. That is a generalisation, not a loss: with several conversations open at once, one note per directory could only ever remember one of them. **No migration of an old note.** A note written by an older olai is not read at all, for either half; it is left on disk for the generic state pruning to treat like any other note, and no plugin name reaches the state package. What that costs a person after the upgrade is one more model switch in the one conversation the note remembered, which `docs/chat.md:125` already describes for a `/model` from before olai watched; say the same there for this upgrade. The `heard/` state stays as it is.
...
**Order of landing is not the numbering.** Land **5.9a (the filer)** immediately after 5.4 and before 5.5: the filer empties the unclaimed set by construction, which retires the Unassigned workflows and their scenarios *before* 5.5 deletes the panel that hosts them, so no temporary host is built and no scenario is left without a face. Then 5.5, 5.6, 5.7, 5.8, then **5.9b (`+ new`)**, which needs the fold and the Recent region, then 5.10. The numbering below is kept so earlier references stay valid.
...
`loadSession` becomes a read-side no-op for node agents (subscribing is opening). Nothing else needs it after 5.9; delete it and its docs. Remove the boot-time auto-open and the which-conversation note's writer and its conversation-half reader per 4.2, and move the model choice to the keyed store 4.2 describes. This was ruled after 5.2 had landed (`26e8cb000`), so it lands as a follow-up commit to 5.2 before 5.9a's filer scenarios, so that from 5.2 on a fresh boot has no live conversation until something reads or holds one; `scoped.test.ts` gets a case that boot acquires no scope, and the e2e idle scenario asserts that a served directory with stored chats reports every agent asleep before any tab unfolds one. Scenarios that stay, re-pointed: `the_agent.feature:915` (a chosen model survives a server restart) becomes "switch the model, restart, open that conversation again, the header names the switched model"; the sibling rules at `docs/chat.md:123-128` (only a switch olai saw is remembered, a switch made elsewhere loses, an agent that will not take the setting is said in a row) keep their scenarios unchanged in substance. Unit: `local.test.ts` or a new `models.test.ts` for the keyed store and its cap, and that an old note present on disk is neither read nor removed by chat.
...
`Ask agent` (`verbs.tsx`): on a row that is an agent or under one, arm the node into the nearest ancestor agent's composer and unfold that agent; on a row with no agent above it, return the refusal sentence "no agent above this row — start one"; when the nearest ancestor agent is **unbound** (in the roster with no session), refuse likewise with "this agent has no session — start one", since there is no `{engine, session}` to unfold and starting one is a write with an engine choice that belongs to the start gesture (the aside pill, the menu entry, or the zoomed composer), never a side effect of asking. **The palette's `>` command keeps the same target rule**, including both refusals: it sends to the nearest ancestor agent (self included) of the focused row, unfolding that agent first so the send is visible where it lands, and its conversation-identity check (`docs/chat.md:163`) is against that agent's current session. With no row focused, or a focused row with no agent above it, it refuses in the palette with the same sentence and keeps the typed text in its input, as a refused `>` does today. It never picks an agent by any other rule. `chat/attention/reveal.ts` (alerts): a notification click still carries only `{kind: "ask"}`; the alerts contract from #578 keeps click payloads free of identity on purpose, and this PR does not touch that union or its decoder. On a click, reveal navigates to and unfolds the **first agent in the roster's needs-you order**, the same order the sidebar's *Needs you* region draws (5.8), so what the click opens is the row at the top of that region. With several agents waiting, the region is the disambiguation, one press away. With none waiting by the time the click lands (the question was answered elsewhere), reveal does nothing beyond focusing the sidebar's *Needs you* region if it is drawn, and otherwise nothing.

Delete: `app.panel` registration, `Panel.tsx`'s `DesktopDock`, mobile sheet, `Minimized.tsx`, `Toggle`, `browser/shell.ts` and `holdShell`, `Choose.tsx` (there is no free chat to choose an engine for), `chat/open.ts`. `Unassigned.tsx` and `showing.ts` are already gone by now (5.9a landed before this item); if any panel-only piece of them survived, delete it here. Remove chat's `lg:pr-[var(--width-panel)]` reservation in `packages/plugins/layout/src/Frame.tsx` only if no other `app.panel` filler exists; otherwise leave layout alone.

e2e: new `node_agent_folds.feature`: unfold draws the transcript and composer under the row; sending from a fold writes the vault through that node's session; two rows unfolded at once each answer their own send; folding releases (the idle reaper can take it; use `@node-idle-fast`); a needs-you question is answered from the fold; a notification click lands on and unfolds the first needs-you agent, and with two waiting it is the one the *Needs you* region lists first (the existing alerts scenarios that asserted the panel opened are re-pointed here; the click payload stays `{kind: "ask"}`); a click with nothing waiting opens no fold; `Ask agent` on a child row arms it in the ancestor's composer; the palette's `>` with a child row focused sends to the ancestor agent and unfolds it, with two folds open it lands in the focused row's ancestor and not the other, and with no agent above the focused row, or an unbound one, it refuses with the matching sentence and keeps the text (re-point the existing `>` scenarios, which asserted on the selected conversation, to this rule); `Ask agent` on a row under an unbound agent refuses the same way and opens no fold; reload folds everything. Move every scenario in `chat_*.feature` that asserted on the dock to the fold or the zoomed page; delete scenarios about the toggle, the sheet, the minimized strip, and `+ new`.
...
Add a **palette adapter** "Agents", registered in navigation's scoped `paletteAdapters` registry (the `app.command` contract takes a text prefix and a send function, not selectable rows, so it is the wrong seam): every node agent as a selectable row, title with standing, in the same activity order, searchable by title, so the agents beyond the cap are one keystroke away; selecting one navigates to the row and unfolds it (an unbound one navigates only, as the sidebar press does). The registration is scoped to chat's browser activation and withdrawn with it, under `AgentsProvider`, reading the same roster.

Delete the old `Agents` section and `focus.ts`'s panel half (keep `rowOf`).

e2e: rewrite `node_agents.feature`'s roster scenarios: the roster is the query still, but drawn as *Needs you* and *Recent*; an asleep agent is in *Recent* with its age and no standing word, and not in *Needs you*; a needs-you agent is in both; ordering by activity; the cap, with the eleventh agent absent from *Recent* and present in the palette's Agents rows; selecting a palette row navigates and unfolds, and an unbound row navigates only; the rows are gone when chat is switched off at the plugins panel and back when it is switched on; `An unbound agent's sidebar press navigates` stays.

### 5.9 Filing unclaimed conversations into the Inbox, automatically

This item lands in two commits. **5.9a** is everything below except the `+ new` paragraph: the filer, its triggers, its writes, what it retires, its unit tests, and the e2e scenarios other than the `+ new` one. It lands right after 5.4, while the panel still exists, and the Unassigned scenarios it replaces are deleted in the same commit, since from this commit the unclaimed set is empty on every fixture with stored sessions and the panel's Unassigned face has nothing to draw. **5.9b** is the `+ new` paragraph and its scenario, landing after 5.8.
...
**Where the Inbox is.** `packages/plugins/capture/src/Inbox.tsx` gets `props.file`; find how capture learns that path and whether the server half of capture knows it too. The runtime has no optional dependency: a component naming an absent service makes the whole row report waiting and blocks browser loading (`packages/plugin-api/src/services.ts:166`). So chat must not name a capture service at all. Instead, **an always-available registry** that capture writes into and chat reads from:

- Check first whether the existing `Offers` service (chat's server already does `offers.own("seating", …)` at `server.ts:209`, its browser `Offers.own("state", …)` at `browser.tsx:128`) is that registry on the server side: owned entries with scoped cleanup, readable by name, absence readable without waiting. If it is, capture's server half owns an `inbox` offer (`{file}`) for the life of its scope and chat reads it at each filer run (or subscribes, if offers are reactive). If `Offers` is browser-only or not readable across plugins, build what Codex described: a registry owned by the vault package (always served, since chat already names `Vault`), `inbox.register(file)` returning a scoped release, `inbox.current()` answering the path or `null`.
- Either way the filer's rule is the same: with no entry there is no filer run, unclaimed conversations stay unclaimed and invisible, and the log says so once at boot and once more when the entry appears or leaves (a plugin switched on or off at the plugins panel). The filer reacts to the entry appearing by running a full run; it does not wait for it.
- Do not make the filer its own row that needs `capture.inbox`: a permanently waiting row on a serve without capture would be a standing warning about a feature nobody turned on.
...
**What it writes.** Through `Ops`, as a sequence of independent writes, because an Ops batch is all-or-nothing (`packages/ops/src/batch.test.ts:345`) and one refused row must not undo the others. First, in its own write: ensure a `Chats` node exists at the top level of the Inbox file (create if absent, reuse if present, matched by a reserved id such as `chats`, never by title). If that write is refused the run stops there and is retried next run. Then one write per unclaimed conversation, in listing order (newest first), each minting one node under `Chats`: title = the conversation's own title (the agent wrote it), or the session id when the listing has none, exactly as the chat list draws a titleless row today; note = `<N> messages · last <minute>` as the listing says it, with a component the listing does not carry left out (`<N> messages` alone, or `last <minute>` alone) and no note at all when both are missing; property = `sessionValue(engine, session)`, always. `/clear` chains: the head of a chain gets the node; the chain behind it is claimed by lineage exactly as `assignSession` claims it today (`docs/chat.md:696-699`), so it never becomes a second node. A refused row is logged with the validator's reason, leaves every other row filed, and is retried next run; a row that succeeded is never written again because the roster check runs per row before its write. Every write is an ordinary op through `Ops.run`, and `Ops.run` requires a writer. None of the existing four fits: `web` and `mcp` are a person's or an agent's request, `chat-agent` would claim an agent wrote what no agent wrote, and `auto` means commits only, never file edits (`packages/format/src/committing.ts:279`). So the vocabulary gains a fifth writer, **`filer`**, declared beside the others in `committing.ts` with its own Commit-panel and ledger label (say what happened in the register the other labels use, along the lines of *olai filed a conversation into the Inbox*, singular and plural), with the existing writer tests extended for it, and with the `auto` rule left exactly as it is. `conversation.newChat` (5.9b) writes as `filer` too, since the mint is the same act; the `startAgentSession` it runs afterwards writes the property as it does today. The writes appear in the Commit panel under that label and commit like any edit. On a directory with many stored chats the first run is many writes in a burst, so what lands in the first commit is whatever the git plugin's cadence gathers, one large commit or a few; say so in `docs/chat.md`.
...
**What it retires (in 5.9a).** `Unassigned.tsx`, `showing.ts`, `assignSession` on the wire and in `binding.ts` (moving the node is the reassignment; `Move to…` keeps the property because it moves the node), `browser/agents/lineage.ts`'s browser-side `unassigned` consumers, and the panel's Unassigned face and its `faceOf` arm in `Panel.tsx`. The panel itself, the `+ new` button and `newSession` stay until 5.5 and 5.9b respectively; `newSession` is deleted in 5.9b if no caller remains. `conversation.sessions` stays for the filer and for 5.7's history.
...
Unit tests (`server/filer.test.ts`): a run mints exactly the unclaimed heads, the `Chats` node in its own write first and then one write per row; a second run mints nothing; a trashed filed node is not re-minted; a reused `Chats` node is not duplicated; a refused `Chats` write files no rows and is retried; a refused row leaves every other row filed, is logged with its reason, and is retried alone next run; a run interrupted between rows leaves the rows already written and files the rest next run; an unreachable agent's chats are skipped and logged; no registry entry means no filer run and one log line, with chat's server row never reporting waiting; an entry appearing later triggers a full run, and an entry leaving stops runs without stopping the row.

e2e (`node_agents.feature`, replacing the Unassigned scenarios; stored-sessions tag). In 5.9a, with the panel still present: a directory with stored chats boots with a `Chats` node in the Inbox carrying one row per chat with the property; every filed row is an asleep node agent at boot, the newest included, since 5.2 removed the boot-time auto-open; a filed row opens in the panel from the sidebar row and answers with its context intact (re-pointed at the fold in 5.5); `Move to…` on it keeps the property and the roster follows; trashing it does not bring it back; the Unassigned row and list are gone from the sidebar and the panel. In 5.9b: `+ new` mints, starts, and unfolds; a `claude --resume` from a terminal (scripted, as the existing stored-sessions scenarios do it) appears in the Inbox after the next settled turn of any agent on that engine, and a settled turn on one engine starts no other engine (with `@codex` installed, the codex adapter is not spawned by a claude turn settling; assert on the scripted adapter's spawn log the way the existing "started to answer, asked, and stopped" scenarios do); the scenario that today forbids a second listing after an already-listed conversation settles is rewritten to assert that narrower rule instead; an agent that could not be asked is named in the log and files nothing.
...
- `docs/git.md` (and wherever else the writer words `web`, `mcp`, `chat-agent`, `auto` are listed, such as `docs/format.md` or `docs/editing.md`; grep for `chat-agent` to find them): add `filer`, what it writes, and that `auto` still never edits a file.
