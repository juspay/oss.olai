...
5. **The sidebar's Agents roster is gone.** Two regions replace it: *Needs you* (standings `needs-you` and `gone` only) and *Recent* (node agents by last activity, capped). *Unassigned* keeps its row, last in *Recent*, drawn only when non-empty. Asleep agents appear nowhere in the sidebar.
...
- The fold line above the transcript (`since <date> · N messages above the fresh start ↑`) is pressable and lists that agent's past sessions using `Conversation.tsx` rows; picking one opens it read-only in the same fold with `current session` to return, matching today's "reading history does not rewrite the property" rule (`docs/chat.md:706-715`). Past sessions come from `pastOf` in `lineage.ts`, unchanged.

e2e: `node_agent_fresh_sessions.feature`, `node_agent_fresh_refusal.feature`, `node_agent_pending_fresh.feature`, `node_agent_removed_fresh.feature`, `node_agent_phone_history.feature`: keep every rule, re-point the steps at the agent line and the fold line. Delete the phone-sheet-specific steps.
...
- **Recent** (`said: "Recent"`): node agents with a `said.at`, ordered by `said.at` desc, then those without one by `node.changed` desc (available on the outlines collection; read it through the roster row's node, or extend `NodeAgentRow` with `changed` on the server side so the browser does one subscription, not two). Cap at 10. Row: `ENTRY_SHAPE` + `AgentMark` + title + `ago` in mono. Current row (`aria-current="page"` when the route is that node's file and the row is unfolded or zoomed) gets the existing `aria-[current=page]` wash. A `new chat` row heads the section (`ENTRY_SHAPE`, `text-paper/65` like the file doors), running `conversation.newChat` (5.9). Draws only the `new chat` row when there are no agents. The old *Unassigned* row is gone (5.9).

Add an `app.command` "Agents" (palette) listing every node agent by title with standing, so asleep agents beyond the cap are one keystroke away; picking one navigates and unfolds.

Delete the old `Agents` section and `focus.ts`'s panel half (keep `rowOf`).

e2e: rewrite `node_agents.feature`'s roster scenarios: the roster is the query still, but drawn as *Needs you* and *Recent*; an asleep agent is in neither; ordering by activity; the cap; the palette command; `An unbound agent's sidebar press navigates` stays.
...
**When it runs.** Owned by the chat server scope, started with it, stopped with it: once after the engines have been probed at boot, and again on every `sessionsRevision` bump (a fresh start, a `+ new`, a settled turn in a conversation the roster does not name). Not on a clock. Each run asks the stored listing the way `conversation.sessions` does (the running agent is asked live; the others are started, asked and stopped, one at a time, with the same few-second reuse). An agent that could not be asked is logged with its reason and retried on the next run; its conversations are neither filed nor forgotten.

**What it writes.** Through `Ops`, one write per run: ensure a `Chats` node exists at the top level of the Inbox file (create if absent, reuse if present, matched by a reserved id such as `chats`, never by title); under it one node per unclaimed conversation, title = the conversation's own title (the agent wrote it), note = `<N> messages · last <minute>` as the listing says it, property = `sessionValue(engine, session)`. Newest first. `/clear` chains: the head of a chain gets the node; the chain behind it is claimed by lineage exactly as `assignSession` claims it today (`docs/chat.md:696-699`), so it never becomes a second node. A refused write for one row leaves the others filed and is retried next run. The write is an ordinary op: it appears in the Commit panel and commits like any edit, which on a directory with many stored chats means one sizeable first commit; say so in `docs/chat.md`.
...
Unit tests (`server/filer.test.ts`): a run mints exactly the unclaimed heads; a second run mints nothing; a trashed filed node is not re-minted; a reused `Chats` node is not duplicated; a refused row leaves the rest filed and is retried; an unreachable agent's chats are skipped and logged; no capture service means no filer and one log line.

e2e (`node_agents.feature`, replacing the Unassigned scenarios; stored-sessions tag): a directory with stored chats boots with a `Chats` node in the Inbox carrying one row per chat with the property; the filed row is an asleep node agent that unfolds and answers with its context intact; `Move to…` on it keeps the property and the roster follows; trashing it does not bring it back; `+ new` mints, starts, and unfolds; a `claude --resume` from a terminal (scripted, as the existing stored-sessions scenarios do it) appears in the Inbox after the next settled turn; an agent that could not be asked is named in the log and files nothing.
