...
`loadSession` becomes a read-side no-op for node agents (subscribing is opening). Nothing else needs it after 5.9; delete it and its docs. Remove the boot-time auto-open and the which-conversation note's writer and reader per 4.2. This was ruled after 5.2 had landed (`26e8cb000`), so it lands as a follow-up commit to 5.2 before 5.9a's filer scenarios, so that from 5.2 on a fresh boot has no live conversation until something reads or holds one; `scoped.test.ts` gets a case that boot acquires no scope, and the e2e idle scenario asserts that a served directory with stored chats reports every agent asleep before any tab unfolds one.
