...
| Quiet rule | The same doctrine #358 ruled for chat: a reminder is for somebody who is **not looking**. If the app is in front of the reader at that moment, the day is marked said and nothing is raised; the burning Agenda entry is what they see. Two facts decide "in front": document visible and window focused. |
...
| Click | Opens `/agenda` in the focused pane. The payload is `{ kind: "due" }`, the second arm of the click union. It names no day, for the reason the union's header gives: what "it" is is a fact the app has when the press lands. |
...
| Ownership | The channel leaves the chat row for a new tab-only row, **`alerts`**, which offers `alerts.channel`; chat's attention circuit and the journal's reminders both name it. This is the one decision the human should ratify before Codex starts (§7, §13). |
...
inputs:  day      the reader's today (ISO day), or "" with no clock
         ...
         watched  document visible AND window focused
...
if consider && watched -> say day, raise nothing           (the app in front of them is the reminder)
if consider && !watched-> say day, then raise: chime, banner
```
...
- **A day switched off is not spent.** With Reminders (or Alerts) off, `said` is not written. Turning it back on the same day, with the window behind, reminds once. This is the honest reading of a switch: off means "do not tell me", not "pretend you told me".
- **Watched consumes the day silently.** This is #358's ruling ("when the pane IS focused, nothing: the form appearing is the alert") applied to the Agenda entry: a reader with olai in front of them has the burning entry, the count chip and the rail dot on screen, and a banner over the app they are looking at is the thing people switch alerts off for. The cost is that a person who opened the app at eight and did not look at the sidebar is not reminded that day. That is the same trade chat made, and one doctrine for both alerts is worth more than a second one for reminders. Flip only if the human says so (§13).
...
| --- | --- | --- |
| Local midnight, tab open on screen | `today()` (the clock's timer); `createOwed` re-opens the stream; the new frame lands | first non-zero frame of the new day; watched or not decides |
| Laptop opened at nine | `today()` re-read on `visibilitychange`; stream re-opens | same |
| Installed app launched at login, window behind | boot; first frame | first non-zero frame; unwatched, so it raises |
...
Two tabs of one olai that are both behind other windows can both receive the same frame at the same instant, both read `said !== day`, and both raise. The banner is one (same tag, the OS replaces); the chime may sound twice. This is the residual `elsewhere.ts` accepted for chat in its own words ("each of those is correctly reporting that nobody is looking, and electing one to speak for the rest is a different mechanism"). The `storage` event closes the window for every later moment. The design does not add an election, and the reviewer should not ask for one.
...
**The click union.** `NotifyClick` becomes `{ kind: "ask" } | { kind: "due" }`. The validator accepts both arms and refuses everything else. Each row subscribes with `onPress` and acts on its own arm only: chat's handler ignores `due`, the journal's ignores `ask`. A stale pre-upgrade envelope is dropped as before.
...
- on, alerts on: "Once a day, when olai is not in front of you, a chime and a notification say what is overdue and on today."
...
export interface Channel {
  ...
  readonly onPress: (press: (asked: NotifyClick) => void) => () => void
  // the devices
...
| --- | --- | --- | --- | --- |
| `alerts` row, `channel` component | the worker click listener and permission listener (`followNotifications`), the two preference followers (`storage` listeners), the audio context and its first-gesture listeners, the badge's `worn` count | `Effect.acquireRelease` in `apply`, offered with `Offers.own("channel")` after acquisition | listeners removed, setters invalidated, audio context closed, badge cleared (`wear(0)`), offer withdrawn | consumers of `alerts.channel` read `waiting`; chat's questions still mark the header toggle; nothing rings |
...
| `chat` row, `attention` component | the attention circuit: the fold over the cell, `createWatching`'s listeners and beat, `createElsewhere`'s channel, the press subscription for `ask` | needs `alerts.channel`, `theme.appearance`-free; holds the channel by identity | circuit disposed, `wear(0)`, press subscription released, `BroadcastChannel` closed | with the alerts row off, the component waits; the panel, toggle and forms are untouched |
| `journal` row, `reminders` component | the reminders circuit (one effect), the `olai.reminders` and `olai.reminders.said` preference followers, the press subscription for `due` | needs `alerts.channel`, `navigation.state`, `layout.deployment`; shares the row's held clock and wire (`holdClocks`, `holdJournalWire`) | effect disposed, followers stopped, press subscription released; `said` stays in storage | with the alerts row off: waits, no reminders, calendar and agenda unaffected; with the journal off: no reading, so nothing; with no clock: `today()` is `""`, nothing is asked |
...
- **The seam is one per origin.** `followNotifications` throws on a second activation, as today. Only the alerts row calls it.
...
- `journal/src/browser/reminders/rule.test.ts`: the table in §3.2, every row, including `day === ""`, `owed === undefined`, both counts zero, `said === day`, off-and-not-spent, watched-and-spent.
...
- `journal/src/browser/claims.test.ts` (new, the shape of `chat/src/browser/claims.test.ts`): no file under `browser/reminders/` spells `setTimeout(` or `setInterval(`; the only speller of `olai:due:` is `reminders/notice.ts`; the only writer of `olai.reminders.said` is `reminders/said.ts`.
...
**The one new stage device: a window that is behind.** Headless Chromium reports every page focused, so `the_agent_waits_on_you.feature` could not drive an unwatched window and settled for a shut panel. Reminders have no panel to shut. So `support/alerts.ts` gains one more last-inch wrapper, on the same terms as its `Notification.permission` one: under a `@behind` tag every document's `Document.prototype.hasFocus` answers `false` from before boot; and two steps, `the window goes behind` and `the window comes to the front`, flip that answer in a running page and dispatch `blur` or `focus` on `window` so both `watching.ts` and the reminders circuit re-read it. What is simulated is a window behind another window, never a state the app invented; `visibilityState` is left alone so the clock's wake and every ticking readout keep running as they really run. The header of `alerts.ts` records this.
...
1. **Launched behind, with work owed, the day is announced once** (`@behind`, a corpus with overdue work such as `good`, whose `order` node is dated 2026-08-10): a notification whose body says `overdue`, tagged `olai:due:<today>` (the step computes today as `isoDayOf(new Date())`, the way `a task is due today` already does), the chime rang, the tab says nothing is waiting, and the Agenda entry burns.
2. **Work that becomes due while nobody is looking** (a scratch with nothing owed, `the window goes behind`, then `a task is due today`): a notification says `1 on today`, the chime rang.
3. **In front of you, the entry is the reminder** (same, window in front): the Agenda entry burns, no notification, no chime, the tab says nothing is waiting; then `the window goes behind` and a second task is dated today: still nothing (the day was spent by being seen).
4. **Once a day** (after 2): a second task dated today raises no second notification and no second chime (`support/alerts.ts` counts both).
5. **A reload says nothing again** (after 2): reload behind, the worker ready, no notification.
6. **A second tab says nothing** (after 2): open a second page in the context behind, no notification in it.
7. **Pressing it opens the agenda** (after 2): `the notification is pressed` (the existing step, with the `due` payload), the pane shows the agenda page.
8. **Reminders off is off** (`the window goes behind`, `I set Reminders to "off"`, a task due today): no notification, no chime, the Agenda entry still burns, `this browser has stored that reminders are "off"`; then `I set Reminders to "on"`: the reminder arrives, because the day was not spent.
9. **Alerts off freezes the Reminders row**: `I set Alerts to "off"`, the Reminders row cannot be set and explains itself.
10. **A browser that refused notifications still chimes** (`@alerts-denied`, `@behind`, work owed): no notification, the chime rang, no tab mark.
11. **The journal off takes the row and the reminder with it** (`the window goes behind`, switch `journal` off on the plugins panel, a task due today: nothing; switch it on: the reminder arrives, the Reminders row is back).
12. **The alerts row off leaves the calendar standing** (`@rows-off:alerts`): the plugins panel says `journal` waits on `alerts.channel` for its reminders component, the preferences panel has no Alerts rows, the calendar and the agenda page still draw, a dated task still lights its day.
...
| `docs/plugins/journal.md` | **Reminders** section: the rule (§3.2 as prose), the moments, the quiet rule, the knob, the pull-only ruling (§10), the `reminders` component and what leaves with it |
...
| `docs/architecture/e2e-coverage.md` | the *Dates and journal* row gains `the_day_reminds_you` and the `@behind` device; the *Preferences and identity* row names the Reminders row |
| `packages/tests/README.md` | the paragraph on what the alerts stage fakes gains `hasFocus` |
...
4. **The stage and the feature.** `support/alerts.ts` gains the `hasFocus` wrapper and the `@behind` tag in `hooks.ts`; `reminders_steps.ts` with the two window steps, the Reminders row steps, the storage steps and the tag assertion; `the_day_reminds_you.feature` with the twelve scenarios of §9.2. Commit: `e2e: the day reminds you, and only once`.
5. **Docs and coverage.** `docs/plugins/journal.md` (Reminders, the ruling), `docs/architecture/overview.md`, `docs/architecture/e2e-coverage.md`, `packages/tests/README.md`. Run the fast remote e2e leg, then `just ci` on the pushed head. Commit: `docs: reminders, and the pull-with-one-push ruling`.

## 13. For the human to decide

One decision, with a recommendation:

- **Ratify the ownership move (§7 A).** It is the larger half of the PR and it touches chat, so it should be a deliberate yes. The alternative (§7 B) is smaller and wrong under the repository's own audit rule; the design is written for A.

Two assumptions the human can flip with a line, and the design absorbs either:

- **Quiet rule.** Watched at the first non-zero reading consumes the day silently (chat's doctrine). The other reading, "raise the banner even in front of the reader, chime only when behind", changes one row of the §3.2 table and scenario 3.
- **Default on.** Matches #358's ruled default for alerts. Default off changes `boolCodec(true)` to `false` and scenario 8's order.

With the move ratified, the design is ready for Codex.
