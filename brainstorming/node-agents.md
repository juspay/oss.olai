...
2. **CONCURRENT AGENTS** (the human, 2026-09-01, after #461's review made the ceiling plain). Today olai runs ONE conversation at a time — `agent.ts` holds `let session: string | null`, and `chat.ts`'s own header says "The one conversation". Everything above it inherits that: an agent that is not the open one **has no process**, so it cannot be working, cannot answer, and cannot act on a wake — which is why #461's roster honestly counts *questions waiting on you* rather than unread messages, and why it needed seven standings (adding **unbound**, **waking**, **gone**) where the design named four. **The ceiling is OURS, not the protocol's:** the pinned ACP adapter already holds many sessions at once — `this.sessions` is keyed by id, `teardownSession(id)` tears down exactly one (`acp-agent.js:3429`), and every `prompt` is addressed by `sessionId`. So the work is olai-side: the agent module holds a MAP of live sessions instead of one; the panel routes turns and deliveries by node rather than by "the" conversation; the roster's live states become true for many rows at once; and session economics (lazy spawn, idle reaping, a cap) become real policy rather than a consequence of there only ever being one. **This phase is what makes a node agent more than a switcher** — everything below it (a wake acted on without a person looking, an agent that dispatches) presumes it.
3. **Migration.** The Unassigned roster entry, the assign-to-node gesture, the distillation order, supersession lineage claiming, the fresh-session affordance. After this phase a person can move their real chats over, one row at a time.
4. **Derived wakes.** A node agent's doorbell scope becomes its subtree, replacing the manual wake control for agent-bound conversations; **an agentless wake CLIMBS to the nearest ancestor node agent** (ruled) — the orchestrator ends up the root catcher, and nothing is silently lost. The manual control survives only for unassigned conversations. Note this phase is only honest AFTER concurrency: until then a wake for an unfocused agent is held, not acted on.
5. **Agency.** A node agent creates child nodes and associates agents with them — dispatch as product (create node, set `agent`, seat a session or a terminal), with the writes-stay-inside-the-subtree rule and stop-and-ask to the ancestor. The layer-3 binding (a node agent ↔ a Spaces bot/thread) rides here too.
6. **The orchestrator migrates LAST** (ruled): once the rest have proven out on smaller agents, the orchestrator conversation is assigned to the orchestrator node — the no-op distillation — and the manual boot-chain era ends as product catches up with practice.
