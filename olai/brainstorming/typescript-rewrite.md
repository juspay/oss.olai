...
2. **See your outline**: serve a directory, parse and validate the `.jsonl` files (format core: unique ids, dangling refs, cycles, stored derived state), render the tree read-only in the browser via a minimal surface view. The store is born this phase in load-once form — design in [architecture.live-store.md](architecture.live-store.md). Visible: your outline in a browser tab — or the validation error, with `file:line`.
3. **It stays live**: probe-based staleness over the recursive directory, snapshot `stream`, error `cell` — the sync loop of [architecture.live-store.md](architecture.live-store.md). Visible: edit a file or `git pull`, the page updates without reload; break a file, the page keeps last-good and shows the error banner.
