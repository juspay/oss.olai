...
4. **The per-frame tax** — 3.5 (coalesce the re-ask), 3.6 (frame counter without clones; may need 5.2 first — if so, count frames with a store version on this side and note it), 3.7, 4.1–4.6, 4.8. **Tests:** `packages/tests/wire.ts` measures calls per frame before/after; unit tests for the `equals` functions.
   ...
   **3.5's remedy was wrong and the finding was right.** The coalescing this asked for was built both ways, measured 9→8 and deleted — a bulk gesture's frames arrive a procedure round trip apart, which is wider than any window a filtered page can honestly hold. The second half (the fresh `Map` per answer) landed with the measurement. The first half landed later and elsewhere: the ask was not too frequent, it was the wrong SHAPE — a standing view asked as a call, over the whole vault, for an answer only ever tested against the page's own rows. It is a stream over the page now, and the gesture costs the matcher nothing (roadmap `filter-ask-carries-revision`, [filter-rides-the-page.md](filter-rides-the-page.md)).
