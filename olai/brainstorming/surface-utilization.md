...
- **Frame cap**: `listener.ts` sets ws `maxPayload` to 8 MiB while the framework's `frameLimit` classifies oversize at 16 MiB. An 8–16 MiB frame dies at the raw ws layer (close 1009, whole socket down) instead of the framework's handled oversize path. Tracked as the `frame-cap` bug.
