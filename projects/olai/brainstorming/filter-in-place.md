...
Two spellings joined that list afterwards and are recorded where they landed, below: the twelve **relative words** (`date:last-week`), and a **duration** back from now (`changed:1h`, `created:30m`), which is the one value here whose bare form is sugar for a range.
...
**Amended by the durations, and the stance is intact — more so, in fact.** A bound is no longer always ten characters, so each bound decides at its OWN width how much of a value it is about: a day bound still reads the day an instant falls on (the old `dayOf`, generalised rather than special-cased), and a duration's bound reads the clock face as well. What makes that free is the property the whole stance rests on and had not yet been spent — ISO text is a **prefix ordering**, so comparing the value whole gives the same answer as comparing its first *n* characters, with one exception at the upper bound (a value sitting *on* the bound and carrying seconds past it, which `startsWith` covers). So the comparison cuts nothing at all, and is cheaper than the single `dayOf` slice that stood there before durations existed. Still text, still no parse; the arithmetic that mints the moment is `calendar.ts`'s, over integers, once per query.
...
**`date:7d` landed too** (`duration-values`), and the sentence above that called it "a second spelling of a span the words already say" was wrong about which span. The words bottom out at a DAY: `today` is since midnight, and the stamps have been carrying seconds the whole time, so `changed:1h` is a question the twelve words cannot ask however they are combined. What landed is `<n><unit>` with four units — `m` minutes, `h` hours, `d` days, `w` weeks — and no month or year unit, which kills the `m` collision by fiat in minutes' favour (month and year recency is already words). A bare duration is sugar for the range it opens, `created:1h` ≡ `created:1h..`, "within the last hour"; a duration at a range's end is the point `now` minus it (`created:..1h`, `changed:2h..30m`, `created:yesterday..3h`). It cost the grammar one new value reading, one new arithmetic in `calendar.ts`, and the width rule under the comparison above — no new operator, no new clause shape, and nothing at any of the five doors, which is the whole return on the matcher living down here.
...
An **impossible duration** joins that paragraph on the same terms, and the case-fold one above it decides which mistakes are even reachable. `created:1mo` and `created:1y` are refused because the units are four and months and years are not among them — a ruling, so the refusal says so in words rather than listing four letters and leaving the reader to try `1mon` next. `created:999999w` is shape-clean and lands nineteen thousand years off the front of what four digits can spell, so it is refused where `date:2026-13` is. And `created:1M` is *not* refused: values are folded, so it is one minute — the collision the ruling settled, surfacing in the one place folding can still surprise somebody.
